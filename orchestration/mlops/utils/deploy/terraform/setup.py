...
"""
    ...
    if re.search(aws_region_pattern, content):
        content = re.sub(
            aws_region_pattern, new_aws_region_block, content, flags=re.DOTALL
        )
    else:
    ...
    print(
        f"Updated {file_path} with aws_region = {aws_region_default} and availability_zones"
    )
