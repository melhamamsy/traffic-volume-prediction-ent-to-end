...
def query_metrics(conn):
    with conn.cursor() as cur:
        cur.execute("""
            SELECT model_id, MIN(timestamp) AS min_timestamp, MAX(timestamp) AS max_timestamp
            FROM metrics
            GROUP BY model_id;
        """)
...
def main(
    ...
    expected_monitor_start_date, expected_monitor_end_date, expected_monitor_report_span
):
    ...
    actual_monitor_start_date = actual_monitor_start_date - pd.DateOffset(years=5) - pd.DateOffset(days=expected_monitor_report_span)
    ...
    assert actual_monitor_start_date == expected_monitor_start_date, "Monitor start date mismatch."
    assert actual_monitor_end_date == expected_monitor_end_date, "Monitor end date mismatch."
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Check database tables and query models and metrics.')
    ...
    main(args.dbname, args.user, args.password, args.host, args.port, args.monitor_start_date, args.monitor_end_date, args.monitor_report_span)