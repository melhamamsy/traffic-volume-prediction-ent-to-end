...
def test_dict_vectorizer_transformer_in_pipeline():
    ...
    pipeline = Pipeline([("dict_vectorizer", DictVectorizerTransformer(sparse=False))])
...
def test_feature_engineering_transformer_in_pipeline():
    data_example = DataFrame(
        {
            "date_time": to_datetime(
                ["2021-12-01 00:00:00", "2021-12-25 12:00:00", "2021-12-31 23:59:00"]
            ),
    ...
    assert transformed_df["weather_main"].tolist() == [
        "Clear",
        "Clouds",
        "Fog_Smoke_Squall",
    ]
...
def test_split_on_date_time():
    ...
    df_train, df_val = split_on_date_time(
        sample_dataframe, "date_time", 2021, drop_feature=True, return_indexes=False
    )
    assert df_train.equals(
        sample_dataframe[sample_dataframe["date_time"].dt.year < 2021].drop(
            columns=["date_time"]
        )
    )
    assert df_val.equals(
        sample_dataframe[sample_dataframe["date_time"].dt.year >= 2021].drop(
            columns=["date_time"]
        )
    )
    ...
    df_train, df_val = split_on_date_time(
        sample_dataframe, "date_time", 2021, drop_feature=False, return_indexes=False
    )
    assert df_train.equals(
        sample_dataframe[sample_dataframe["date_time"].dt.year < 2021]
    )
    assert df_val.equals(
        sample_dataframe[sample_dataframe["date_time"].dt.year >= 2021]
    )
    ...
    train_index, val_index = split_on_date_time(
        sample_dataframe, "date_time", 2021, drop_feature=False, return_indexes=True
    )
    expected_train_index = sample_dataframe[
        sample_dataframe["date_time"].dt.year < 2021
    ].index
    expected_val_index = sample_dataframe[
        sample_dataframe["date_time"].dt.year >= 2021
    ].index
